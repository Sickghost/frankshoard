...
use frankshoard::{
    Argon2Conf, BasicPasswordEntry, Config, Entry, Error, LockedHoard, NoteEntry, SiteEntry,
    UIConf, UnlockedHoard,
};
...
#[command(
    about = "A secure password manager used to store secrets (password and notes) along with related data."
)]
struct Cli {
...
enum Commands {
    ...
    /// Prints the given entry (except secrets)
    Entry {
        #[arg(long)]
        id: Uuid,
    },
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let password = Zeroizing::new(Password::new().with_prompt("Master password").interact()?);
...
fn add(
    mut unlocked_hoard: UnlockedHoard,
    entry_type: AddCommands,
    silent: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    match entry_type {
        AddCommands::BasicPassword {
            entry_name,
            username,
        } => {
            // Ask for password
...
fn entry_note(
    ...
    match entry {
        ...
        Entry::Site(site) => match site.note()? {
            Some(n) => println!("{}", *n),
            None => {
                if !silent {
                    println!("No note")
