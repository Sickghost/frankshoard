...
///
/// We also want to avoid leaking password length, so we use a hardcoded length value for password.
///
...
pub const MAX_PASSWORD_SIZE: usize = 256;

#[derive(Debug, ZeroizeOnDrop)]
pub struct PasswordBuf(Box<[u8; MAX_PASSWORD_SIZE]>);

impl PasswordBuf {
    pub fn new(password: Zeroizing<String>) -> Result<Self, Error> {
        let bytes = password.as_bytes();
        if bytes.len() > MAX_PASSWORD_SIZE {
            return Err(Error::SecretTooLong);
        }
        let mut pwd_buf = Box::new([0u8; MAX_PASSWORD_SIZE]);
        pwd_buf[..bytes.len()].copy_from_slice(bytes);

        Ok(PasswordBuf(pwd_buf))
    }

    pub fn as_str(&self) -> Result<Zeroizing<String>, Error> {
        let len = self.0.iter().position(|&b| b == 0).unwrap_or(MAX_PASSWORD_SIZE);
        let s = std::str::from_utf8(&self.0[..len]).map_err(|_| Error::CorruptedSecret)?;
        Ok(Zeroizing::new(s.to_string()))
    }
}

impl Zeroize for PasswordBuf {
    fn zeroize(&mut self) {
        self.0.iter_mut().for_each(|b| *b = 0);
    }
}

impl Serialize for PasswordBuf {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        serializer.serialize_bytes(self.0.as_ref())
    }
}

impl<'de> Deserialize<'de> for PasswordBuf {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let mut pwd_buf = Box::new([0u8; MAX_PASSWORD_SIZE]);
        pwd_buf.copy_from_slice(&bytes);
        Ok(PasswordBuf(pwd_buf))
    }
}

#[derive(Debug, ZeroizeOnDrop)]
pub struct NoteBuf(Box<[u8]>);

impl NoteBuf {
    pub fn new(note: Zeroizing<String>) -> Result<Self, Error> {
        let bytes = note.as_bytes();
        let note_buf = bytes.to_vec().into_boxed_slice();
        Ok(NoteBuf(note_buf))
    }
...
impl Zeroize for NoteBuf {
    fn zeroize(&mut self) {
        self.0.iter_mut().for_each(|b| *b = 0);
    }
}

impl Serialize for NoteBuf {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        serializer.serialize_bytes(self.0.as_ref())
    }
}

impl<'de> Deserialize<'de> for NoteBuf {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let bytes = <Vec<u8>>::deserialize(deserializer)?;
